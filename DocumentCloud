...
You must also have this installed and ready to go: [MuckRock/documentcloud-frontend: DocumentCloud's front end source code - Please report bugs, issues and feature requests to info@documentcloud.org (github.com)](https://github.com/muckrock/documentcloud-frontend)
...
Make sure you in your **Squarelet** `.envs/.local/.django` file, there exist values for: `STRIPE_PUB_KEYS`, `STRIPE_SECRET_KEYS`, and set `STRIPE_WEBHOOK_SECRETS=None`. Multiple values for any of the above should be comma delimited.

You should now be able to log in to DocumentCloud using your Squarelet account.

### Installation of DocumentCloud
...
7. Click save and continue editing. In  `.envs/.local/.django` (**in the DocumentCloud project**) set the following environment variables:
